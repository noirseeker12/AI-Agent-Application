...
# 设置streamlit标题
st.markdown("<h1 style='text-align: center; color: Black;'>基于Web URL的问答</h1>", unsafe_allow_html=True)
# 创建三个列，用于布局
...
def get_bot_response(user_query):
    ...
    # 将刚刚添加的HumanMessage从messages列表中移除。这样做的原因是 main_content 包含了用户的原始查询和相似文本内容
    # 但在实际的对话历史中，我们只希望保留用户的原始查询和AI响应，而不是包含相似文本的查询
    messages.pop()
    # 将用户查询添加到消息列表
...
# 创建一个显示消息的函数
def display_content(all_messages):
    for msg in all_messages:
        if msg['user'] == 'user':
            message(f"You ({msg['time']}): {msg['text']}", is_user=True, key=str(time.time_ns()))
        else:
            message(f"wiki-bot ({msg['time']}): {msg['text']}", key=str(time.time_ns()))
...
messages = [
    SystemMessage(
        content="你是一个问答机器人，你将回答用户的所有问题。如果你不知道答案输出“对不起，我不知道”"
    )
]
...
if len(search):
    ...
    if len(wiki_content):
        try:
            ...
            if len(user_query) and send_btn:
                # 初始化向量数据库
            ...
            if st.session_state.doc_search is not None:
                ...
                display_content(st.session_state.all_messages)
